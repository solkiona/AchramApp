...
const handleApiError = (errorObj: any): string => {
    console.log("Handling API error object:", errorObj);

    const responseData = errorObj?.response?.data || errorObj;

    if (responseData && responseData.status === "error") {
        const details = responseData.details || {};
        const nonFieldErrors = Array.isArray(details.non_field_errors) ? details.non_field_errors : [];
        const { non_field_errors, ...fieldErrorsDetails } = details;
        
        const parsedFieldErrors = parseFieldErrors(fieldErrorsDetails);
        setFieldErrors(parsedFieldErrors);

        let message = responseData.message || 'Operation failed.';
        ...
        setGeneralError(message);
        return message; // <--- ADD THIS
    } 
    else if (responseData && responseData.message) {
        setGeneralError(responseData.message);
        setFieldErrors({});
        return responseData.message; // <--- ADD THIS
    } 
    else {
        const fallbackMsg = errorObj?.message || 'An unexpected error occurred.';
        setGeneralError(fallbackMsg);
        setFieldErrors({});
        return fallbackMsg; // <--- ADD THIS
    }
};
